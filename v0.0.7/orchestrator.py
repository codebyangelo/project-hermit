...
class Orchestrator:
    ...
    def discover_and_register_new_skill(self) -> Tuple[bool, str]:
        """
        Scans the workspace codebases (project-lobster, project-mantis), extracts computational
        functions using AST, prompts the Discovery Agent to select the most relevant bottleneck skill,
        ...
        # Prioritize active versions of lobster and mantis to ensure we scan the current files
        prioritized_dirs = [
        ...
        for sdir in search_dirs:
            for root, dirs, files in os.walk(sdir):
                # Skip hidden, pycache and project-hermit itself
                dirs[:] = [d for d in dirs if not d.startswith('.') and d != '__pycache__' and d != 'project-hermit']
                for file in files:
                    if file.endswith('.py'):
