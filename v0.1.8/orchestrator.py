...
class Orchestrator:
    ...
    def detect_oscillation(self, skill_name: str, window: int = 10) -> bool:
        """Detects if optimization strategies are oscillating (newest-first, reversed to chronological)."""
        conn = self._get_conn()
        cursor = conn.cursor()
        cursor.execute("""
        ...
        # Also detect returning to recently banned/attempted strategy after short exploration (A -> B -> A where B failed or was attempted)
        if len(strategies) >= 3:
    ...
    def ban_strategies(self, skill_name: str, strategies: List[str], cooldown: int = 5):
        if not self.detect_oscillation(skill_name):
            return
