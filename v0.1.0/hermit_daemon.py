        ...
        while True:
            try:
                ...
                if task:
                    ...
                    if skills:
                        # Sort skills so we prioritize optimizing the lowest versions first (balancing optimizations)
                        skills.sort(key=lambda x: x[2])  # x[2] is version
                        target = skills[0][0]
                        ...
                        success, msg = self.orchestrator.run_evolution_step(target, harness)
