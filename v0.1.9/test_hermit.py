...
class TestProjectHermit(unittest.TestCase):
    ...
    def test_v019_oscillation_cycle_detection(self):
        """Verifies true cycle detection patterns."""
        skill_name = "test_cycle_skill"
        self.orchestrator.register_or_update_skill(skill_name, "desc", "def test_cycle_skill(): pass")
        
        # Scenario 1: A-B-A-B period-2 cycle
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        for i, strat in enumerate(["strat_a", "strat_b", "strat_a", "strat_b"]):
            cursor.execute("""
                INSERT INTO version_history (skill_name, version, code, strategy, latency_ms)
                VALUES (?, ?, ?, ?, ?)
            """, (skill_name, i+1, "pass", strat, 10.0))
        conn.commit()
        conn.close()
        
        is_cycle, to_ban = self.orchestrator.detect_oscillation(skill_name, window=6)
        self.assertTrue(is_cycle)
        self.assertCountEqual(to_ban, ["strat_a", "strat_b"])

    def test_v019_sandbox_security_hardening(self):
        """Verifies sandbox safety limits and dangerous import blocking."""
        # Block subprocess/os.system
        res = run_in_sandbox("import subprocess\nprint('bad')", "security_test.py")
        self.assertEqual(res.exit_code, -3)
        self.assertIn("[SANDBOX SECURITY ERROR] blocked_import: subprocess", res.stderr)
        
        res = run_in_sandbox("import os\nos.system('ls')", "security_test.py")
        self.assertEqual(res.exit_code, -3)
        self.assertIn("[SANDBOX SECURITY ERROR] blocked_call: os.system", res.stderr)

    def test_v019_memory_accuracy(self):
        """Verifies fork-based non-zero RSS changes."""
        code = "x = [0]*100000; print('allocated')"
        res = run_in_sandbox(code, "mem_test.py")
        self.assertEqual(res.exit_code, 0)
        self.assertGreater(res.max_rss_kb, 0)

    def test_v019_pareto_dominates(self):
        """Verifies hierarchical Pareto gates prevent regressions."""
        baseline = {"latency": 100.0, "rss": 5000, "complexity": 100}
        
        # Case 1: Latency regression (exceeds 5% noise margin)
        cand_reg = {"latency": 110.0, "rss": 4500, "complexity": 50}
        is_improved, reason = self.orchestrator.pareto_dominates(cand_reg, baseline)
        self.assertFalse(is_improved)
        self.assertEqual(reason, "latency_regression")
        
        # Case 2: Memory regression (exceeds 10% margin)
        cand_mem = {"latency": 90.0, "rss": 6000, "complexity": 50}
        is_improved, reason = self.orchestrator.pareto_dominates(cand_mem, baseline)
        self.assertFalse(is_improved)
        self.assertEqual(reason, "memory_regression")
        
        # Case 3: Insignificant improvement (none improved by >= 5%)
        cand_insig = {"latency": 99.0, "rss": 4900, "complexity": 99}
        is_improved, reason = self.orchestrator.pareto_dominates(cand_insig, baseline)
        self.assertFalse(is_improved)
        self.assertEqual(reason, "insignificant_improvement")
        
        # Case 4: Valid improvement
        cand_good = {"latency": 90.0, "rss": 4900, "complexity": 99}
        is_improved, reason = self.orchestrator.pareto_dominates(cand_good, baseline)
        self.assertTrue(is_improved)
