...
class HermitDaemon:
    def __init__(self, db_path: str = DB_PATH):
        ...
        self._daemon_startup_time = datetime.datetime.now(datetime.timezone.utc)
        self._last_stagnation_trigger_time = datetime.datetime.now(datetime.timezone.utc)
    ...
    def check_stagnation(self):
        """Checks if the system has stagnation (no merges in the last 1 hour) and triggers forced exploration."""
        conn = self._get_conn()
        cursor = conn.cursor()
        cursor.execute("""
            SELECT timestamp FROM version_history 
            WHERE strategy != 'v1' 
        ...
        if row:
            try:
                ts_str = row[0]
                if 'T' in ts_str:
                    last_merge = datetime.datetime.fromisoformat(ts_str)
                else:
                    # SQLite CURRENT_TIMESTAMP format
                    last_merge = datetime.datetime.strptime(ts_str, "%Y-%m-%d %H:%M:%S").replace(tzinfo=datetime.timezone.utc)
            except Exception:
                last_merge = datetime.datetime.now(datetime.timezone.utc)
        else:
            last_merge = getattr(self, '_daemon_startup_time', datetime.datetime.now(datetime.timezone.utc))
            
        now = datetime.datetime.now(datetime.timezone.utc)
        
        last_trigger = getattr(self, '_last_stagnation_trigger_time', None)
        if last_trigger:
            minutes_since_trigger = (now - last_trigger).total_seconds() / 60.0
            if minutes_since_trigger < 30.0:
                return

        hours_since_merge = (now - last_merge).total_seconds() / 3600.0
        
        if hours_since_merge > 1.0:
            self._last_stagnation_trigger_time = now
            ExecutionLogger.log("DAEMON", f"STAGNATION_WARNING: No merges in {hours_since_merge:.2f} hours. Enabling forced exploration mode.", "WARN")
            self.force_exploration_mode()
            
    def force_exploration_mode(self):
        ExecutionLogger.log("DAEMON", "FORCED EXPLORATION MODE: Cleared complex categorizations and triggering new skill discovery...", "INFO")
    ...
    def get_bottleneck_skills(self) -> List[Tuple[str, float]]:
        ...
        for name, desc in rows:
            ...
            if self.consecutive_failures.get(name, 0) >= 3:
                continue
            if desc and ("RESEARCH_STATUS" in desc or "EXPLORED" in desc):
                continue
            if not self.orchestrator.can_attempt_skill(name):
                self.mark_skill_explored(name, reason="budget_exhausted")
                continue
    ...
    def get_next_target(self) -> str:
        ...
        # Filter out excluded, complex, failed, or budget-exhausted skills
        viable_skills = []
