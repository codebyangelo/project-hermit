        ...
        while True:
            try:
                ...
                if task:
                    ...
                    if skills:
                        # Select target skill (default to hex_search, otherwise the first registered)
                        target = None
                        for s in skills:
                            if s[0] == "hex_search":
                                target = s[0]
                        ...
                        self.orchestrator.run_evolution_step(target, DEFAULT_HARNESS)
                        
                        # Check context decay conditions
                        self.check_and_apply_context_decay(target)
                    else:
                        ExecutionLogger.log("DAEMON", "No active skills registered in the database. Standing by...")
