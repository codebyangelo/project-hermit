...
def get_tool_manifest():
    ...
    return {
        "tools": [
            ...
            {
                "name": "mutate_math_in_code",
                "description": "Applies rule-based AST optimizations to simplify mathematical expressions in the provided Python code.",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "code": {"type": "string", "description": "The source code of the Python function to mutate."}
                    },
                    "required": ["code"]
                }
            },
            {
                "name": "mutate_qubo_in_code",
                "description": "Applies rule-based AST reductions to simplify QUBO matrix multiplications and double-loop quadratic equations in Python code.",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "code": {"type": "string", "description": "The source code of the Python function to mutate."}
...
def mutate_math_in_code(code):
    if not code:
        return {"status": "error", "content": "Parameter 'code' is required."}
    try:
        sys.path.append(os.path.dirname(os.path.abspath(__file__)))
        import math_mutator
        optimized = math_mutator.optimize_math_ast(code)
        if optimized:
            return {"status": "success", "content": optimized, "mutated": True}
            
        return {"status": "success", "content": code, "mutated": False}
    except Exception as e:
        return {"status": "error", "content": f"Failed to run math mutation: {str(e)}"}

def mutate_qubo_in_code(code):
    if not code:
        return {"status": "error", "content": "Parameter 'code' is required."}
    try:
        sys.path.append(os.path.dirname(os.path.abspath(__file__)))
        import qubo_mutator
        optimized = qubo_mutator.optimize_qubo_ast(code)
        if optimized:
            return {"status": "success", "content": optimized, "mutated": True}
            
        return {"status": "success", "content": code, "mutated": False}
    except Exception as e:
        return {"status": "error", "content": f"Failed to run QUBO mutation: {str(e)}"}
...
def execute_tool(name, arguments):
    ...
    handlers = {
        ...
        "mutate_math_in_code": lambda args: mutate_math_in_code(args.get("code")),
        "mutate_qubo_in_code": lambda args: mutate_qubo_in_code(args.get("code"))
    }
