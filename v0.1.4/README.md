# Project Hermit v0.1.4 - Recursive Self-Improvement Architecture

Version `v0.1.4` extends the self-improvement architecture with intelligent target cooldown, retry pivots, and database-driven research status categorization.
...
8. **Intelligent Target Bypassing & Pivots:**
   ...
   * Applies dynamic pivot instructions (e.g. type-safety, algorithmic simplifications) on retries to escape local optima.
   * Categorizes plateaued skills as `COMPLEX_NEEDS_FURTHER_RESEARCH` in description, bypassing them in the daemon loop to focus on other unbenchmarked bottleneck targets.
