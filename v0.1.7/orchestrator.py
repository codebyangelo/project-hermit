...
class Orchestrator:
    ...
    def select_api_key(self) -> str:
        ...
        for k in self.api_keys:
            ...
            # Reset RPM every 60s
            if now_time - stats['last_rpm_reset'] >= 60.0:
                stats['rpm_used'] = 0
                stats['last_rpm_reset'] = now_time
        ...
        for k in self.api_keys:
            ...
            if stats['rpm_used'] < RPM_LIMIT and stats['tpm_used'] < TPM_LIMIT and stats['daily_tpm'] < DAILY_TPM_CAP:
                viable.append(k)
                
        if not viable:
            ExecutionLogger.log("LLM_CLIENT", "All keys exhausted or rate-limited (Limits: RPM < 12, TPM < 200k, Daily < 1M). Sleeping for 15s...", "WARN")
            time.sleep(15.0)
            return self.select_api_key()
            
        # Weighted: prefer least daily_tpm used
        chosen = min(viable, key=lambda k: self.key_stats[k]['daily_tpm'])
    ...
    def record_usage(self, key: str, tokens: int):
        ...
        if stats:
            stats['rpm_used'] += 1
            stats['tpm_used'] += tokens
            stats['daily_tpm'] += tokens

    def record_failure(self, key: str):
        stats = self.key_stats.get(key)
        if stats:
            stats['rpm_used'] = 999  # block it until next reset (60s)
            stats['tpm_used'] = 999999
    ...
    def call_gemini_api(self, prompt: str, system_instruction: Optional[str] = None) -> Dict[str, Any]:
        ...
        for attempt in range(1, max_retries + 1):
            ...
            current_key = self.select_api_key()
            try:
    ...
    def run_evolution_step(self, skill_name: str, verification_harness: str, user_instruction: Optional[str] = None) -> Tuple[bool, str]:
        ...
        for var in variants:
            ...
            if not any_failed and candidate_latencies:
                ...
                score = (avg_latency * 1.0) + (avg_rss * 0.1) + (complexity * 0.05)
                
                # Check for statistically significant speed improvement or general RAM/complexity improvements
                is_stat_faster = self.is_significant_improvement(baseline_latencies, candidate_latencies)
                is_improved = (
                    is_stat_faster or
                    avg_rss < baseline_result.max_rss_kb or
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("""
                SELECT branch_name FROM skill_branches 
                WHERE skill_name = ? AND status = 'merged' 
                ORDER BY timestamp DESC LIMIT 10
            """, (skill_name,))
            recent_strategies = [r[0] for r in cursor.fetchall()]
            conn.close()
            
            recent_strategies = [best["branch_name"]] + recent_strategies
            if len(recent_strategies) > 10:
                recent_strategies = recent_strategies[:10]
            ...
            if cycle_detected:
                ...
                for s in recent_strategies:
                    if s not in unique_strats:
                        unique_strats.append(s)
                    if len(unique_strats) >= 3:
                        break
                self.ban_strategies(skill_name, unique_strats, cooldown=5)
                ExecutionLogger.log("ORCHESTRATOR", f"OSCILLATION_DETECTED for '{skill_name}': Banned strategies {unique_strats} for next 5 attempts.", "WARN")
        except Exception as e:
