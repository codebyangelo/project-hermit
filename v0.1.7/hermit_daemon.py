...
class HermitDaemon:
    def __init__(self, db_path: str = DB_PATH):
        ...
        self.start_time_sqlite = datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')
    ...
    def get_next_target(self) -> str:
        """Determines the next target skill using a fairness quota strategy.
        70% cycles: Bottleneck sort (slowest passing skills)
        20% cycles: Zero-merge skills (untouched)
        10% cycles: Random exploration
        """
        import random
        ...
        # 1. 20% of cycles: zero-merge skills
        if 0.7 <= r < 0.9:
            placeholders = ",".join("?" for _ in valid_skills)
            valid_names = [v[0] for v in valid_skills]
            cursor.execute(f"""
                SELECT DISTINCT skill_name FROM skill_branches 
                WHERE status = 'merged' AND skill_name IN ({placeholders})
            """, valid_names)
            merged_skills = {row[0] for row in cursor.fetchall()}
            untouched_skills = [name for name, _ in valid_skills if name not in merged_skills]
            if untouched_skills:
                chosen_skill = random.choice(untouched_skills)
                strategy = "Zero-Merge (Fairness Quota)"
        
        # 2. 10% of cycles: random exploration
        elif r >= 0.9:
            chosen_skill = random.choice([v[0] for v in valid_skills])
            strategy = "Random Exploration"
            
        # 3. 70% of cycles or fallback (if no zero-merge found or r < 0.7)
        if not chosen_skill:
            skills_with_latency = []
            ...
            skills_with_latency.sort(key=lambda x: (x[1] == 0.0, x[1]), reverse=True)
            chosen_skill = skills_with_latency[0][0] if skills_with_latency else None
            strategy = f"Bottleneck Sort (Latency: {skills_with_latency[0][1]:.2f}ms)" if skills_with_latency else "Bottleneck Sort"
            
        conn.close()
        if chosen_skill:
            ExecutionLogger.log("DAEMON", f"Selected target '{chosen_skill}' using strategy: {strategy}", "INFO")
        return chosen_skill
        ...
        while self.should_continue():
            try:
                ...
                if rpm >= 12 or tpm >= 200000:
                    ...
                    if temp > 75.0:
                        ExecutionLogger.log("DAEMON", f"THERMAL_THROTTLE: CPU Temperature is {temp:.1f}°C (> 75°C). Cooling down for 60s...", "WARN")
                        self.broadcast_status("Throttled (Thermal: Cool Down)")
                        time.sleep(60.0)
                    elif temp > 65.0:
                        ExecutionLogger.log("DAEMON", f"THERMAL_THROTTLE: CPU Temperature is {temp:.1f}°C (> 65°C). Cooling down for 30s...", "WARN")
                        self.broadcast_status("Throttled (Thermal: Moderate)")
                        time.sleep(30.0)
                    elif temp > 55.0:
                        ExecutionLogger.log("DAEMON", f"THERMAL_THROTTLE: CPU Temperature is {temp:.1f}°C (> 55°C). Cooling down for 10s...", "WARN")
