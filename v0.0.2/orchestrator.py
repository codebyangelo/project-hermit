...
class Orchestrator:
    ...
    def run_evolution_step(self, skill_name: str, verification_harness: str, user_instruction: Optional[str] = None) -> Tuple[bool, str]:
        ...
        if baseline_result.exit_code != 0:
            err = f"Baseline implementation failed verification tests! Stderr: {baseline_result.stderr}"
            ExecutionLogger.log("ORCHESTRATOR", err, "ERROR")
            return False, err
        ...
        if not self.has_api_access():
            err = "No GEMINI_API_KEY detected. Skipping evolutionary query."
        ...
        try:
            ...
            err = f"Failed to parse mutation JSON response: {e}"
            ExecutionLogger.log("ORCHESTRATOR", err, "ERROR")
        ...
        if mutation_result.exit_code != 0:
            err = f"Mutation failed verification tests! Stderr: {mutation_result.stderr}"
            ExecutionLogger.log("ORCHESTRATOR", err, "ERROR")
            return False, err
        ...
        if mutation_result.duration_ms < baseline_result.duration_ms:
            ...
            msg = f"INTEGRATION SUCCESS: Mutation is faster by {time_saved:.2f} ms!"
            ...
            msg = f"INTEGRATION SUCCESS: Mutation saved {ram_saved} KB of RAM!"
            ExecutionLogger.log("ORCHESTRATOR", msg, "SUCCESS")
            self.register_or_update_skill(skill_name, desc, optimized_code)
            return True, msg
        else:
            msg = "INTEGRATION REJECTED: Mutation did not beat baseline metrics."
