...
def print_help():
    ...
    print("  (e.g., 'Optimize hex_search for 20% less RAM' or 'Speed up pattern matching')\n")
...
def inject_task(prompt):
    conn = get_conn()
    cursor = conn.cursor()
    # Default to targeting hex_search for the default harness
    cursor.execute("""
    ...
    conn.commit()
    conn.close()
    print(f"\n[SUCCESS]: Optimization task injected into queue (Status: pending).")
